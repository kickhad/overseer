...
{
	public class UMMachineProvider : MachineProvider<UMMachine> 
    {
		readonly RestMachineConnector<UMMachine> _connector;

		public UMMachineProvider(UMMachine machine)
			: this(new RestMachineConnector<UMMachine>(), machine)
		{
		}

        public UMMachineProvider(RestMachineConnector<UMMachine> connector, UMMachine machine)
		{
			Machine = machine;
		...
		public override async Task LoadConfiguration(Machine machine)
        {
            try
            {
				var updatedMachine = machine as UMMachine;
